...
for grupo, partidos in calendario.items():
    ...
    for p in partidos:
        cols = st.columns([3, 2, 0.5, 2, 3])
        cols[0].write(f"P{p[0]} | {p[1]}")
        cols[1].write(f"**{p[2]}**")
        loc = cols[2].number_input("L", key=f"l{p[0]}", label_visibility="collapsed")
        cols[2].markdown('<div class="vs-text">vs</div>', unsafe_allow_html=True)
        vis = cols[3].number_input("V", key=f"v{p[0]}", label_visibility="collapsed")
        cols[4].write(f"**{p[3]}**")
