...
for grupo, juegos in calendario.items():
    with st.expander(grupo):
        for juego in juegos:
            ...
            # Sin keys dinámicas para evitar errores de renderizado
            loc = st.number_input(f"L{juego[0]}", min_value=0, step=1)
            cols[3].markdown('<p class="vs-texto">vs</p>', unsafe_html=True)
            vis = st.number_input(f"V{juego[0]}", min_value=0, step=1)
...
with c1:
    if st.button("💾 GUARDAR JSON"):
        if nombre:
            data = {"nombre": nombre, "datos": pronosticos}
            st.download_button("📥 Descargar JSON", json.dumps(data), "quiniela.json")
with c2:
    if st.button("📄 GENERAR PDF"):
        if nombre:
            st.download_button("📥 Descargar PDF", generar_pdf(nombre, pronosticos), "quiniela.pdf", mime="application/pdf")
with c3:
